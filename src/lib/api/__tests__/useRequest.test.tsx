...
describe('API - useRequest hook', () => {
    ...
    test('Should handle network error', async () => {
        ...
        expect(getByText(/error/)).toHaveTextContent(
            /request to http:\/\/server.fakeapi.com\/ failed, reason: server error/,
        )
    })
    ...
    test('Should refetch when the path changes', async () => {
        const firstCall = nock('http://server.fakeapi.com')
            .get('/')
            .reply(200, {id: 1})
        const secondCall = nock('http://server.fakeapi.com')
            .get('/next')
            .reply(200, {id: 2})
    ...
    test("Should not refetch if the path doesn't change", () => {
        let apiCalls = 0
